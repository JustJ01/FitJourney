...
import { Mail, User, Briefcase, Type, Award } from 'lucide-react';
      ...
      <Card className="shadow-xl">
        <CardHeader className="items-center text-center">
          <Avatar className="h-24 w-24 mb-2 border-4 border-primary">
            {(user as any).avatarUrl && <AvatarImage src={(user as any).avatarUrl} alt={user.name || 'User Avatar'} />}
        ...
        <CardContent className="space-y-6">
          ...
          {isTrainer && (user as any).bio && (
            <div className="space-y-2 p-3 bg-muted/30 rounded-md">
              <div className="flex items-center space-x-3">
                <Type className="h-6 w-6 text-primary" />
                <div>
                  <p className="text-sm text-muted-foreground">Bio</p>
                  <p className="font-medium text-md italic">{(user as any).bio}</p>
                </div>
              </div>
            </div>
          )}

          {isTrainer && (user as any).specializations && (user as any).specializations.length > 0 && (
            <div className="space-y-2 p-3 bg-muted/30 rounded-md">
              ...
              <div className="flex flex-wrap gap-2">
                {(user as any).specializations.map((spec: string, index: number) => (
